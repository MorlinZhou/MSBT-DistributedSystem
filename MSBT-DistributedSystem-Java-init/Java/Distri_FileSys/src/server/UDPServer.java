...
//        }
        if ("READ".equals(command)) {
            ...
            // Call the searchFile method to search for the file
            String response = searchFile(fileName);
            return response;
        } else if ("CREATE".equals(command)) {
            String fileName = parts[1];
            String response = createFile(fileName);
            return response;
    ...
    private String searchFile(String fileName) {
        File[] files = new File("path/to/search/directory").listFiles();
        for (File file : files) {
